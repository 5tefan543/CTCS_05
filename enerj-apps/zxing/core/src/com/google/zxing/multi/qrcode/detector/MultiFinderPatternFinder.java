...
final class MultiFinderPatternFinder extends FinderPatternFinder {
  ...
  private static class ModuleSizeComparator implements Comparator {
    public int compare(Object center1, Object center2) {
      @Approx float value = ((FinderPattern) center2).getEstimatedModuleSize() -
                    ((FinderPattern) center1).getEstimatedModuleSize();
      return Endorsements.endorse(value) < 0.0 ? -1 : Endorsements.endorse(value) > 0.0 ? 1 : 0;
    }
  }
  ...
  private FinderPattern[][] selectBestPatterns() throws NotFoundException {
    ...
    for (int i1 = 0; i1 < (size - 2); i1++) {
      ...
      for (int i2 = i1 + 1; i2 < (size - 1); i2++) {
        ...
        @Approx float vModSize12 = (p1.getEstimatedModuleSize() - p2.getEstimatedModuleSize()) /
            (ApproxMath.min(p1.getEstimatedModuleSize(), p2.getEstimatedModuleSize()));
        @Approx float vModSize12A = ApproxMath.abs(p1.getEstimatedModuleSize() - p2.getEstimatedModuleSize());
        if (Endorsements.endorse(vModSize12A > DIFF_MODSIZE_CUTOFF && vModSize12 >= DIFF_MODSIZE_CUTOFF_PERCENT)) {
        ...
        for (int i3 = i2 + 1; i3 < size; i3++) {
          ...
          // Compare the expected module sizes; if they are really off, skip
          @Approx float vModSize23 = (p2.getEstimatedModuleSize() - p3.getEstimatedModuleSize()) /
              (ApproxMath.min(p2.getEstimatedModuleSize(), p3.getEstimatedModuleSize()));
          @Approx float vModSize23A = ApproxMath.abs(p2.getEstimatedModuleSize() - p3.getEstimatedModuleSize());
          if (Endorsements.endorse(vModSize23A > DIFF_MODSIZE_CUTOFF && vModSize23 >= DIFF_MODSIZE_CUTOFF_PERCENT)) {
            // break, since elements are ordered by the module size deviation there cannot be
            // any more interesting elements for the given p1.
            break;
          }
          ...
          // Check the sizes
          @Approx float estimatedModuleCount = ((dA + dB) / p1.getEstimatedModuleSize()) / 2;
          if (Endorsements.endorse(estimatedModuleCount > MAX_MODULE_COUNT_PER_EDGE ||
  ...
  public FinderPatternInfo[] findMulti(Hashtable hints) throws NotFoundException {
    ...
    for (int i = iSkip - 1; i < maxI; i += iSkip) {
        ...
        if (Endorsements.endorse(image.get(j, i))) {
          ...
          if ((currentState & 1) == 0) { // Counting black pixels
            if (currentState == 4) { // A winner?
              if (Endorsements.endorse(foundPatternCross(stateCount))) { // Yes
                @Approx boolean confirmed = handlePossibleCenter(stateCount, i, j);
                if (!confirmed) {
      ...
      if (Endorsements.endorse(foundPatternCross(stateCount))) {
